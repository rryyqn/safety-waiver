        ...
        return (
          <Fragment key={step.name}>
            ...
            <div className="flex items-center gap-2 flex-col h-fit w-16 z-10">
              <div
                className={cn(
                  "size-10 rounded-full flex items-center justify-center border-2 transition-colors bg-background",
            ...
            {!isLastStep && (
              <div
                className={cn(
                  "flex-1 h-[2px] -mx-4 -mt-6 transition-colors duration-300",
                  // Line is colored if the NEXT step is completed or active
                  currentStepIndex > index ? "bg-primary" : "bg-muted/30"
                )}
              />
            )}
          </Fragment>
