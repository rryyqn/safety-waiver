...
import { CircleX, SearchAlert } from "lucide-react";
import Link from "next/link";
import { Suspense } from "react";

export default async function AdminDashboard({ searchParams }: { searchParams: Promise<{search?: string; from?: string; to?: string; range?: string; page?: string}>; }) {
  ...
  return (
        ...
        <div className="flex flex-col gap-12 h-full">
          ...
          <div className="absolute right-0 top-0 flex flex-row justify-end gap-1 items-center text-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 px-4 py-3 pr-12 text-sm">
...
async function WaiverList({ params }: { params: {search?: string; from?: string; to?: string; range?: string; page?: string} }) {
  ...
  if (waivers.length === 0) {
    return (
      <div className="w-full h-full flex flex-col justify-center items-center text-sm gap-2"><SearchAlert className="size-12 text-muted " strokeWidth={1.5} /><p className="text-muted">No waivers found</p><Button variant="secondary" size="dashboard" asChild><Link href="/admin" className=""><CircleX className="size-4" />Clear Filters</Link></Button></div>
    );
  }
...
async function WaiverPages({ params }: { params: {search?: string; from?: string; to?: string; range?: string; page?: string} }) {
